...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("marxcoin"))
        return false;
...
{
    // Convert marxcoin:// to marxcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("marxcoin://"))
    {
        uri.replace(0, 12, "marxcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "MarxCoin.lnk";
}
...
{
    return GetAutostartDir() / "marxcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("MarxCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  marxcoin-qt [" + tr("command-line options") + "]                     " + "\n";
